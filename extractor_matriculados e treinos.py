...
for linha in resultados:
    ...
    # algum codigo que conte o total geral
    total_geral = total_geral + linha[1]
    # armazenar os dados formatados na variavel relatorio
    relatorio.append(f'Treino {str(linha[0]).zfill(3)} -> {linha[1]} alunos matriculados')
    
# Geração de relatório de texto
data_geracao = datetime.now().strftime("%d/%m/%Y %H:%M")
nome_arquivo = "relatorio_matriculas.txt"

with open(nome_arquivo,"w", encoding="utf-8") as arquivo:
    arquivo.write("=== RELATÓRIO DE MATRÍCULAS FITLIFE ===\n")
    arquivo.write(f"Gerado em: {data_geracao}\n")
    arquivo.write("----------------------------------------\n")
    #codigo para escrever os dados do relatorio no arquivo
